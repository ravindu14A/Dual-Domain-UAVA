...
Camera: taken from Test_time.py configs. All three configs (water, air, turbine)
        must use the same camera_type — a ValueError is raised otherwise.
"""
...
# ── 1. Camera consistency check ───────────────────────────────────────────────
_types = {water_config["camera_type"],
          air_config["camera_type"],
          turbine_config["camera_type"]}
if len(_types) > 1:
    raise ValueError(
        f"All phase configs must use the same camera_type. Found: {_types}\n"
        "Set water_config, air_config, and turbine_config to the same camera."
    )

CAM_TYPE = water_config["camera_type"]
cam_base = cameras[CAM_TYPE]

if CAM_TYPE == "HYPERSPECTRAL" and "v_fov" not in cam_base:
    _SPIRAL_OK = False
    print("[D_sweep] HYPERSPECTRAL selected — spiral mode disabled (no v_fov).")
else:
    _SPIRAL_OK = True

print(f"[D_sweep] Camera: {CAM_TYPE}  |  Spiral enabled: {_SPIRAL_OK}")
...
def _v_scan(cam, D, R, v_max, mode):
    """Return scan speed [m/s] for the given camera, standoff, and mode."""
    with contextlib.redirect_stdout(io.StringIO()):   # suppress w_arc cap warnings
...
def section(R_s, R_t, H_s, D, cam, v_max, v_horiz, mode, n=1):
    """
...
for i, D in enumerate(D_arr):
    cam = {**cam_base, "D": D}   # only D is swept; all other camera params fixed

    # ── section 1: underwater ──
    d1_lm, t1_lm = section(R_base, R_base, H_water, D, cam,
                            v_max_uw, v_horiz_uw, "lawnmower")
    if _SPIRAL_OK:
        d1_sp, t1_sp = section(R_base, R_base, H_water, D, cam,
                                v_max_uw, v_horiz_uw, "spiral")
    ...
    d2_lm, t2_lm = section(R_base, R_base, H_air_cyl, D, cam,
                            v_max_air, v_horiz_air, "lawnmower")
    if _SPIRAL_OK:
        d2_sp, t2_sp = section(R_base, R_base, H_air_cyl, D, cam,
                                v_max_air, v_horiz_air, "spiral")

    # transition: air cylinder → cone: zero (continuous)
    d_tr2, t_tr2 = 0.0, 0.0

    # ── section 3: air cone ──
    d3_lm, t3_lm = section(R_base, R_top, H_air_cone, D, cam,
                            v_max_air, v_horiz_air, "lawnmower")
    if _SPIRAL_OK:
        d3_sp, t3_sp = section(R_base, R_top, H_air_cone, D, cam,
                                v_max_air, v_horiz_air, "spiral")
    ...
    d4_lm, t4_lm = section(R_blade, R_blade, H_blade, D, cam,
                            v_max_turb, v_horiz_turb, "lawnmower", n=3)
    if _SPIRAL_OK:
        d4_sp, t4_sp = section(R_blade, R_blade, H_blade, D, cam,
                                v_max_turb, v_horiz_turb, "spiral", n=3)
...
# current D from Test_time
D_current = cam_base["D"]

fig1, ax1 = plt.subplots(figsize=(9, 5))
ax1.plot(D_arr, dist_lm / 1e3, **_STYLE["lawnmower"])
if _SPIRAL_OK:
    ax1.plot(D_arr, dist_sp / 1e3, **_STYLE["spiral"])
ax1.axvline(D_current, color="white", lw=1.0, ls="--", alpha=0.6,
            label=f"Current D = {D_current} m")
ax1.set_xlabel("Standoff distance D  [m]", fontsize=11)
ax1.set_ylabel("Total path distance  [km]", fontsize=11)
ax1.set_title(f"Total inspection distance vs standoff  ({CAM_TYPE} camera)", fontsize=12)
...
ax2.axvline(D_current, color="white", lw=1.0, ls="--", alpha=0.6,
            label=f"Current D = {D_current} m")
ax2.set_xlabel("Standoff distance D  [m]", fontsize=11)
ax2.set_ylabel("Total mission time  [hours]", fontsize=11)
ax2.set_title(f"Total mission time vs standoff  ({CAM_TYPE} camera)", fontsize=12)
